...
export const createComment = async (params: CreateCommentParams) => {
    ...
    */
		...
		const user = await User.findById(updatedPost.author, "name settings").exec();
		const {notification_comment} = user.settings;

		if (isOwnAction && notification_comment !== false) {
			const messageForUser = `🔔 ${user.name} прокомментировал ваш пост - ${updatedPost.title}`;

			await pusherServer.trigger(updatedPost.author.toString(), "comment", messageForUser);
		}
