...
const NotificationPage = async ({searchParams}: any) => {
	...
	return (
		<div className='mx-auto w-full max-w-5xl pb-6 pt-[85px] max-[1280px]:px-4'>
			<h1 className='text-4xl font-bold'>Уведомления ({result?.notifications.length})</h1>
			...
			<div className='mt-6 flex flex-col gap-1'>
				{result?.notifications.length > 0 ? (
										...
										>
											...
											<div className='flex-1 rounded-lg border border-neutral-800 p-5'>
												<div className='mb-5 flex items-center gap-3'>
													...
													>
														<p>{item.user.name}</p>
														<p className='text-sm text-zinc-400'>@{item.user.username}</p>
													</Link>
					...
					<div className='h-[394px] w-full gap-1.5 bg-main flex flex-col items-center justify-center rounded-md p-10 text-center'>
